...
except the daily digest and per-event reminders below, both of which post
automatically (deliberate, narrow exceptions; see `CLAUDE.md` invariant 1). One bot
process and one admin cover any number of registered groups — see
...
```
  ...
  node-cron ──────────────────────────────────┘
     ...
     every 5 min: event reminders (~15 min before a confirmed event's time) →
                  posted directly, no approval
```
